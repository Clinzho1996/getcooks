...
export const sendCustomOrderToCustomer = async (customer, cook, order) => {
	const formattedPaymentLink = `https://getameal.app/pay/${order._id}?kitchen=${cook.storeHandle}&link=${encodeURIComponent(order.paymentLink)}`;
	const receiptUrl = `https://getameal.app/receipt/${order._id}?phone=${order.customerPhone}`;
